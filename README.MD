...
- **`full_gallery`** *(boolean, optional)*: When `true`, return every image in the listing gallery. When `false` (default), return only the hero image.
...
##### Example Request with `full_gallery`

```http
GET /inserat/12345?full_gallery=true
```
